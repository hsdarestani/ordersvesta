<?php
/**
 * Plugin Name: Vesta Bot Bridge - Media Fix
 * Description: Adds chunked signed-GET image upload to Vesta Bot Bridge, avoiding external downloads blocked by Zhaket Booster/WAF.
 * Version: 1.0.0
 * Author: Vesta
 * Requires PHP: 7.4
 */

if (!defined('ABSPATH')) {
    exit;
}

function vbbmf_tmp_dir() {
    $uploads = wp_upload_dir();
    $dir = trailingslashit($uploads['basedir']) . 'vesta-bot-bridge-tmp';
    if (!is_dir($dir)) {
        wp_mkdir_p($dir);
    }
    return $dir;
}

function vbbmf_id($payload) {
    $id = isset($payload['upload_id']) ? strtolower((string) $payload['upload_id']) : '';
    if (!preg_match('/^[a-f0-9]{32}$/', $id)) {
        vbb_fail('Invalid upload id.', 400);
    }
    return $id;
}

function vbbmf_path($id) {
    return trailingslashit(vbbmf_tmp_dir()) . $id . '.part';
}

function vbbmf_meta_key($id) {
    return 'vbb_upload_' . $id;
}

function vbbmf_result_key($id) {
    return 'vbb_upload_result_' . $id;
}

function vbbmf_b64decode($value) {
    $value = strtr((string) $value, '-_', '+/');
    $pad = strlen($value) % 4;
    if ($pad) {
        $value .= str_repeat('=', 4 - $pad);
    }
    return base64_decode($value, true);
}

function vbbmf_begin($payload) {
    $id = vbbmf_id($payload);
    $filename = sanitize_file_name(isset($payload['filename']) ? $payload['filename'] : 'vesta-product.jpg');
    $size = isset($payload['size']) ? intval($payload['size']) : 0;
    $sha256 = isset($payload['sha256']) ? strtolower((string) $payload['sha256']) : '';

    if ($size <= 0 || $size > 10 * 1024 * 1024) {
        vbb_fail('Invalid media size.', 400);
    }
    if (!preg_match('/^[a-f0-9]{64}$/', $sha256)) {
        vbb_fail('Invalid media hash.', 400);
    }

    $existing = get_transient(vbbmf_result_key($id));
    if (is_array($existing) && !empty($existing['id'])) {
        return array('upload_id' => $id, 'already_finished' => true, 'result' => $existing);
    }

    $path = vbbmf_path($id);
    $fh = @fopen($path, 'c+b');
    if (!$fh) {
        throw new Exception('Could not initialize media upload.');
    }
    if (!flock($fh, LOCK_EX)) {
        fclose($fh);
        throw new Exception('Could not lock media upload.');
    }
    ftruncate($fh, 0);
    fflush($fh);
    flock($fh, LOCK_UN);
    fclose($fh);

    set_transient(vbbmf_meta_key($id), array(
        'filename' => $filename,
        'size' => $size,
        'sha256' => $sha256,
    ), HOUR_IN_SECONDS);

    return array('upload_id' => $id, 'ready' => true);
}

function vbbmf_chunk($payload) {
    $id = vbbmf_id($payload);
    $meta = get_transient(vbbmf_meta_key($id));
    if (!is_array($meta)) {
        vbb_fail('Upload session expired.', 410);
    }

    $offset = isset($payload['offset']) ? intval($payload['offset']) : -1;
    $bytes = vbbmf_b64decode(isset($payload['data']) ? (string) $payload['data'] : '');
    if ($offset < 0 || $bytes === false || $bytes === '') {
        vbb_fail('Invalid media chunk.', 400);
    }
    if ($offset + strlen($bytes) > intval($meta['size'])) {
        vbb_fail('Media chunk exceeds declared size.', 400);
    }

    $path = vbbmf_path($id);
    $fh = @fopen($path, 'c+b');
    if (!$fh) {
        throw new Exception('Could not open media upload.');
    }
    if (!flock($fh, LOCK_EX)) {
        fclose($fh);
        throw new Exception('Could not lock media upload.');
    }
    if (fseek($fh, $offset) !== 0) {
        flock($fh, LOCK_UN);
        fclose($fh);
        throw new Exception('Could not seek media upload.');
    }
    $written = fwrite($fh, $bytes);
    fflush($fh);
    flock($fh, LOCK_UN);
    fclose($fh);

    if ($written !== strlen($bytes)) {
        throw new Exception('Could not write complete media chunk.');
    }
    return array('upload_id' => $id, 'offset' => $offset, 'written' => $written);
}

function vbbmf_finish($payload) {
    $id = vbbmf_id($payload);
    $existing = get_transient(vbbmf_result_key($id));
    if (is_array($existing) && !empty($existing['id'])) {
        return $existing;
    }

    $meta = get_transient(vbbmf_meta_key($id));
    if (!is_array($meta)) {
        vbb_fail('Upload session expired.', 410);
    }
    $path = vbbmf_path($id);
    if (!is_file($path)) {
        vbb_fail('Uploaded media file not found.', 404);
    }

    clearstatcache(true, $path);
    if (filesize($path) !== intval($meta['size'])) {
        vbb_fail('Uploaded media size mismatch.', 409);
    }
    if (!hash_equals((string) $meta['sha256'], (string) hash_file('sha256', $path))) {
        @unlink($path);
        delete_transient(vbbmf_meta_key($id));
        vbb_fail('Uploaded media checksum mismatch.', 409);
    }

    require_once ABSPATH . 'wp-admin/includes/file.php';
    require_once ABSPATH . 'wp-admin/includes/media.php';
    require_once ABSPATH . 'wp-admin/includes/image.php';

    $file = array('name' => (string) $meta['filename'], 'tmp_name' => $path);
    $attachment_id = media_handle_sideload($file, 0);
    if (is_wp_error($attachment_id)) {
        @unlink($path);
        throw new Exception($attachment_id->get_error_message());
    }

    delete_transient(vbbmf_meta_key($id));
    $result = array(
        'id' => (int) $attachment_id,
        'source_url' => (string) wp_get_attachment_url($attachment_id),
    );
    set_transient(vbbmf_result_key($id), $result, HOUR_IN_SECONDS);
    return $result;
}

add_action('template_redirect', function () {
    // Main Vesta Bot Bridge 1.1+ must be active. At runtime all plugins are loaded,
    // so its HMAC authorization helpers are available regardless of plugin load order.
    if (!function_exists('vbb_v2_request') || !function_exists('vbb_v2_authorize') || !vbb_v2_request()) {
        return;
    }

    $raw_op = isset($_GET['o']) ? sanitize_key(wp_unslash($_GET['o'])) : '';
    if (!in_array($raw_op, array('media_begin', 'media_chunk', 'media_finish'), true)) {
        return;
    }

    list($op, $payload) = vbb_v2_authorize();
    try {
        if ($op === 'media_begin') {
            $result = vbbmf_begin($payload);
        } elseif ($op === 'media_chunk') {
            $result = vbbmf_chunk($payload);
        } else {
            $result = vbbmf_finish($payload);
        }
        vbb_no_cache();
        wp_send_json_success($result);
    } catch (Throwable $e) {
        vbb_fail($e->getMessage(), 500);
    }
    exit;
}, -1);
